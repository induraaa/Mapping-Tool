...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Wrap analytics content in a scroll area so nothing gets clipped when
        # the panel is shorter than its content (e.g. at 125 % DPI scaling).
        ...
        _gb_m = max(4, round(7 / _s))   # tighter groupbox inner margin
        av = QVBoxLayout(self.analytics_panel)
        av.setContentsMargins(_ap, _ap, _ap, _ap)
        av.setSpacing(max(4, round(7 / _s)))
        ...
        self.cpk_label.setStyleSheet(f'color:{T["text_secondary"]};font-size:{max(10, round(12 / _s))}px;')
        top_row.addWidget(self.cpk_label, stretch=1)
        av.addLayout(top_row)

        dist_box = QGroupBox('Distribution')
        dv = QVBoxLayout(dist_box); dv.setContentsMargins(_gb_m, _gb_m, _gb_m, _gb_m); dv.setSpacing(4)
        self.hist_panel = HistogramPanel()
        # Let the panel's own DPI-aware minimum height govern; just cap the max.
        self.hist_panel.setMaximumHeight(max(140, round(190 / _s)))
        dv.addWidget(self.hist_panel)
        av.addWidget(dist_box)

        yield_box = QGroupBox('Pass / Fail Summary')
        ov = QVBoxLayout(yield_box); ov.setContentsMargins(_gb_m, _gb_m, _gb_m, _gb_m); ov.setSpacing(4)
